...
namespace BZ10
{
    ...
    public partial class MainForm : Form
    {
        ...
        GlobalParam transferParam = new GlobalParam();
        private double wd;
        private double sd;
        ...
        public void NetServerInit()
        {
            try
            {
                ...
                {
                    DebOutPut.DebLog("首次连接传输数据服务器！");
                    DebOutPut.WriteLog(LogType.Normal, LogDetailedType.Ordinary, "首次连接传输数据服务器！");
                    transferParam.devid = Param.DeviceID;
                    transferParam.host = Param.TransferUploadIP;
                ...
                try
                {
                    ...
                    for (int i = 0; i < UploadTable.Rows.Count; i++)
                    {
                        ...
                        if (Param.NetworkCommunication == "0")//Socket通讯方式
                        {
                            ...
                            if (Param.IsTransfer == "1")
                            {
                                if (transferClient != null && transferClient.clientSocket != null && transferClient.clientSocket.Connected && TransferClient.newDateTime.AddMinutes(5) > DateTime.Now)
                                {
                                    bool isSuccess = transferClient.SendPicMsg(dt.ToString(Param.dataType, System.Globalization.DateTimeFormatInfo.InvariantInfo), path);
                                    if (isSuccess)
                                    {
                                        DateTime startTime = DateTime.Now;
                                        bool isUpladResult = true;//本张图像服务器回应结果
        ...
        */
        ...
        public void dealTransferMsg(string jsonText)
        {
            lock (SequenceLockdealMsg)
            {
                int func = -1;
                try
                {
                    if (jsonText != "")
                    {
                        ...
                        func = Convert.ToInt16(jo["func"].ToString());
                        if (func == 100)
                            DebOutPut.WriteLog(LogType.Normal, LogDetailedType.KeepAliveLog, "传输数据Socket事件_接收:" + jsonText);
                        else
                            DebOutPut.WriteLog(LogType.Normal, LogDetailedType.Ordinary, "传输数据Socket事件_接收:" + jsonText);
                        DebOutPut.DebLog("Socket事件_接收:" + jsonText);
                        switch (func)
                        {
                            case 100://连接保持帧头回应
                            ...
                            case 101: //发送采集信息 上位机→服务器
                                DebOutPut.DebLog("发送采集信息收到回应，指令:101");
                                DebOutPut.WriteLog(LogType.Normal, LogDetailedType.Ordinary, "收到传输数据采集信息上传回复！");
                                isUpladCom = true;
                                break;
                            case 102: //发送位置信息 上位机→服务器
                                      //DebOutPut.DebLog( "发送位置信息收到回应，指令:102");
...
, isbug);
                            ...
                            case 111: //获取设备当前状态和当前动作 服务器→上位机
                                ...
                                if (position == "")
                                    position = "等待";
                                transferClient.senddevStatus(111, "", statusInfo, position);
                                DebOutPut.DebLog("同步载破片数量和工作模式");
                                int remain1 = int.Parse(this.TxtRemain.Text.Trim());//载玻片数量
                                if (remain1 < 0)
                                    remain1 = 0;
                            ...
                            case 131: //远程开关设备  服务器→上位机
                                string mess1 = jo["message"].ToString();
                                transferClient.Replay(131, "success", "");
                                if (mess1.ToUpper() == "ON")
                                {
                                    if (!devIsStart)
                                    {
                            ...
                            case 143: //设备状态切换：正常、调试 服务器→上位机
                                string msg1 = jo["message"].ToString();
                                transferClient.Replay(143, "success", "");
                                if (msg1 == "1")
                                {
                                    NormalMode();
                                    this.cb_switch.Checked = false;
                            ...
                            case 137: //推片 服务器→上位机
                                ...
                                {
                                    if (label21.Text == "存在")
                                        transferClient.Replay(137, "fail", "载玻片已存在");
                                    else
                                    {
                                    ...
                                    transferClient.Replay(137, "fail", "只有调试模式下才可使用此功能");
                                break;
                            case 138: //滴加粘附液 服务器→上位机
                                if (cb_switch.Checked)
                                {
                                    ...
                                    transferClient.Replay(138, "fail", "只有调试模式下才可使用此功能");
                                break;
                            case 139: //收集 服务器→上位机
                                if (cb_switch.Checked)
                                {
                                    ...
                                    transferClient.Replay(139, "fail", "只有调试模式下才可使用此功能");
                                break;
                            case 140: //滴加培养液 服务器→上位机
                                if (cb_switch.Checked)
                                {
                                    ...
                                    transferClient.Replay(140, "fail", "只有调试模式下才可使用此功能");
                                break;
                            case 126: //拍照  服务器→上位机
                                if (cb_switch.Checked)
                                {
                                    ...
                                    transferClient.Replay(126, "fail", "只有调试模式下才可使用此功能");
                                break;
                            case 141: //回收载玻片 服务器→上位机
                                if (cb_switch.Checked)
                                {
